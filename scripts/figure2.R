...
poldat.lm.gg <- poldat.lm %>% 
  ...
  geom_pointrange() +
  scale_colour_manual(values=c('yes'='darkorange', 'no'='#999999')) +
...
temp.gg <- hadisst.summary %>% 
  ...
  scale_color_manual(values=c('#0072B2','navy')) +
...
fig1A <- grid.arrange(temp.gg, pol.assemb.gg, eq.assemb.gg, ncol=1)
ggsave(fig1A, filename=here("results","fig1_part1.png"),height=8, width=4, dpi=300, scale=1.1)
fig1B <- grid.arrange(poldat.lm.gg, eqdat.lm.gg, ncol=2)
ggsave(fig1B, filename=here("results","fig1_part2.png"),height=8, width=7, dpi=300, scale=0.9)
